...
public class StepTracker {
    ...
    void printStepsMounth(int keyMonth) {// Печать за месяц Количества пройденных шагов по дням
    ...
    void sumStepsMounth(int keyMonth) {//Метод Общее количество шагов за месяц
        int sumAllSteps = 0;
        int[] daysInMonth = monthTableStep.get(keyMonth - 1);
        for (int amountSteps : daysInMonth) {
    ...
    void averStepsMount(int keyMonth) {//Метод Среднее количество шагов;
        int sumAllStep = 0;
        int averageSteps = 0;
        int[] daysInMonth = monthTableStep.get(keyMonth - 1);
        for (int amountSteps : daysInMonth) {
    ...
    void bestSeries(int keyMonth) {//Лучшая серия: максимальное количество подряд идущих дней
        int day = 0;
        int max = 0;
        int[] daysInMonth = monthTableStep.get(keyMonth - 1);
